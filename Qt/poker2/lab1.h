...
long long ModularExponentiation(long long a,long long x, long long p)
{
    long long result = 1;
    long long s;
    s=a;
...
void DiffiHellman()
{
    ...
    Xa=Random();
    Ya=ModularExponentiation(g,Xa,p);

    Xb=Random();
    Yb=ModularExponentiation(g,Xb,p);

    printf("Ya=%lli, Yb=%lli\n",Ya,Yb);

    Zab=ModularExponentiation(Yb,Xa,p); //A
    Zba=ModularExponentiation(Ya,Xb,p); //B
...
/*	map<long long,int>::iterator it;
     for (it = babyStep.begin(); it != babyStep.end(); ++it)
  {
    ...
    for(int i=1;i<=k;i++)
    {
        long long s=ModularExponentiation(a,i*m,p);
